...
#import "BmobSocketIOTransportXHR.h"
...
//    NSURLConnection *conn;
    for (NSString *key in _polls) {
        ...
        task = [pollData objectForKey:@"connection"];
        [task cancel];
...
//    [pollData setObject:conn forKey:@"connection"];
    [pollData setObject:task forKey:@"connection"];
    [pollData setValue:data forKey:@"data"];
    [_polls setObject:pollData forKey:task.description];
...
-(void)URLSession:(NSURLSession *)session task:(NSURLSessionTask *)task didCompleteWithError:(NSError *)error{
    if (error) {
        ...
        NSMutableDictionary *pollData = [_polls objectForKey:session.description];
        NSString *data = [pollData objectForKey:@"data"];
        [_polls removeObjectForKey:session.description];
        ...
        if ([retries intValue] < 2) {
            ...
            if ([delegate respondsToSelector:@selector(onError:)]) {
                [delegate onError:[NSError errorWithDomain:bmobkSocketIOError
