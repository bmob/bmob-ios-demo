...
-(void)saveInBackground:(BmobBooleanResultBlock)block
    ...
    if (![SDKHostUtil upyunVersion] || [[SDKHostUtil upyunVersion] intValue] < [BmobManager defaultManager].upyunVersion) {
...
-(void)uploadFileWithFilename:(NSString *)filename
    ...
    [BmobUPYUNConfig sharedInstance].DEFAULT_BUCKET = self.upyunCDN.domin;//@"test654123";
    ...
    weakUY.successBlocker = ^(NSURLResponse *response, id responseData) {
        ...
        if ([self.upyunCDN.domin rangeOfString:@"https://"].location != NSNotFound) {
            self.url = [self.upyunCDN.domin stringByAppendingPathComponent:self.path];
        }else{
            self.url = [NSString stringWithFormat:@"https://%@",[self.upyunCDN.domin stringByAppendingPathComponent:self.path]] ;
        }
