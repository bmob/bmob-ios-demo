...
@interface NSData (BmobSRWebSocket)
...
@interface NSString (BmobSRWebSocket)
...
@interface NSURL (BmobSRWebSocket)
...
@interface _bSRRunLoopThread : NSThread
...
static NSString *newSHA1String(const char *bytes, size_t length) {
    ...
    return  [[NSString alloc] initWithData:[[NSData dataWithBytes:md length:CC_SHA1_DIGEST_LENGTH] base64EncodedDataWithOptions:0] encoding:NSUTF8StringEncoding];
    //  [[NSData dataWithBytes:md length:CC_SHA1_DIGEST_LENGTH] base64Encoding];
}

@implementation NSData (BmobSRWebSocket)
...
@implementation NSString (BmobSRWebSocket)
...
NSString *const bmobSRWebSocketErrorDomain = @"BmobSRWebSocketErrorDomain";

// Returns number of bytes consumed. Returning 0 means you didn't match.
// Sends bytes to callback handler;
typedef size_t (^stream_scanner)(NSData *collected_data);

typedef void (^data_callback)(BmobSRWebSocket *webSocket,  NSData *data);

@interface BSRIOConsumer : NSObject {
...
@interface BSRIOConsumerPool : NSObject

- (id)initWithBufferCapacity:(NSUInteger)poolSize;

- (BSRIOConsumer *)consumerWithScanner:(stream_scanner)scanner handler:(data_callback)handler bytesNeeded:(size_t)bytesNeeded readToCurrentFrame:(BOOL)readToCurrentFrame unmaskBytes:(BOOL)unmaskBytes;
- (void)returnConsumer:(BSRIOConsumer *)consumer;

@end

@interface BmobSRWebSocket ()  <NSStreamDelegate>
...
@implementation BmobSRWebSocket {
    ...
    __strong BmobSRWebSocket *_selfRetain;
    
    NSArray *_requestedProtocols;
    BSRIOConsumerPool *_consumerPool;
}
...
- (void)_SR_commonInit;
{
    ...
    _consumerPool = [[BSRIOConsumerPool alloc] init];
...
- (void)_HTTPHeadersDidFinish;
{
    ...
    if(![self _checkHandshake:_receivedHTTPHeaders]) {
        [self _failWithError:[NSError errorWithDomain:bmobSRWebSocketErrorDomain code:2133 userInfo:[NSDictionary dictionaryWithObject:[NSString stringWithFormat:@"Invalid Sec-WebSocket-Accept response"] forKey:NSLocalizedDescriptionKey]]];
        return;
    }
    ...
    if (negotiatedProtocol) {
        ...
        if ([_requestedProtocols indexOfObject:negotiatedProtocol] == NSNotFound) {
            [self _failWithError:[NSError errorWithDomain:bmobSRWebSocketErrorDomain code:2133 userInfo:[NSDictionary dictionaryWithObject:[NSString stringWithFormat:@"Server specified Sec-WebSocket-Protocol that wasn't requested"] forKey:NSLocalizedDescriptionKey]]];
            return;
        }
...
- (void)_readHTTPHeader;
{
    ...
    [self _readUntilHeaderCompleteWithCallback:^(BmobSRWebSocket *self,  NSData *data) {
...
- (void)didConnect
{
    ...
    _secKey = [[NSString alloc] initWithData:[keyBytes base64EncodedDataWithOptions:0] encoding:NSUTF8StringEncoding];
//    keyBytes.base64Encoding;
...
- (void)_handleFrameHeader:(frame_header)frame_header curData:(NSData *)curData;
{
    ...
    if (frame_header.payload_length == 0) {
        ...
        [self _addConsumerWithDataLength:(size_t)frame_header.payload_length callback:^(BmobSRWebSocket *self, NSData *newData) {
            if (isControlFrame) {
...
- (void)_readFrameContinue;
{
    ...
    [self _addConsumerWithDataLength:2 callback:^(BmobSRWebSocket *self, NSData *data) {
        ...
        if (extra_bytes_needed == 0) {
            ...
            [self _addConsumerWithDataLength:extra_bytes_needed callback:^(BmobSRWebSocket *self, NSData *data) {
...
- (BOOL)_innerPumpScanner {
   ...
   // size_t curSize = readBufferSize - _readBufferOffset;
    ...
    BSRIOConsumer *consumer = [_consumers objectAtIndex:0];
...
@implementation BSRIOConsumerPool {
...
- (BSRIOConsumer *)consumerWithScanner:(stream_scanner)scanner handler:(data_callback)handler bytesNeeded:(size_t)bytesNeeded readToCurrentFrame:(BOOL)readToCurrentFrame unmaskBytes:(BOOL)unmaskBytes;
{
    BSRIOConsumer *consumer = nil;
    if (_bufferedConsumers.count) {
        consumer = [_bufferedConsumers lastObject];
        [_bufferedConsumers removeLastObject];
    } else {
        consumer = [[BSRIOConsumer alloc] init];
    }
...
@implementation NSURL (BmobSRWebSocket)
...
static _bSRRunLoopThread *networkThread = nil;
static NSRunLoop *networkRunLoop = nil;

@implementation NSRunLoop (BmobSRWebSocket)

+ (NSRunLoop *)SR_networkRunLoop {
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        networkThread = [[_bSRRunLoopThread alloc] init];
...
@implementation _bSRRunLoopThread {
