...
//  Copyright 2011, -fno-objc-arc2012 OpenUDID.org
//
...
#import <CommonCrypto/CommonDigest.h> // Need to import for CC_SHA256 access
...
static NSString * bmobkOpenUDIDSessionCache = nil;
//static NSString * const bmobkOpenUDIDDescription = @"OpenUDID_with_iOS6_Support";
static NSString * const bmobkOpenUDIDKey = @"OpenUDID";
static NSString * const bmobkOpenUDIDSlotKey = @"OpenUDID_slot";
static NSString * const bmobkOpenUDIDAppUIDKey = @"OpenUDID_appUID";
static NSString * const bmobkOpenUDIDTSKey = @"OpenUDID_createdTS";
static NSString * const bmobkOpenUDIDOOTSKey = @"OpenUDID_optOutTS";
static NSString * const bmobkOpenUDIDDomain = @"org.OpenUDID";
static NSString * const bmobkOpenUDIDSlotPBPrefix = @"org.OpenUDID.slot.";
static int const kOpenUDIDRedundancySlots = 100;

@interface BmobOpenUDID (Private)
...
+ (void) _setDict:(id)dict forPasteboard:(id)pboard {
#if TARGET_OS_IPHONE || TARGET_IPHONE_SIMULATOR		
    [pboard setData:[NSKeyedArchiver archivedDataWithRootObject:dict] forPasteboardType:bmobkOpenUDIDDomain];
#else
...
+ (NSMutableDictionary*) _getDictFromPasteboard:(id)pboard {
#if TARGET_OS_IPHONE || TARGET_IPHONE_SIMULATOR	
    id item = [pboard dataForPasteboardType:bmobkOpenUDIDDomain];
#else
...
//
+ (NSString*) value {
    return [BmobOpenUDID valueWithError:nil];
}

+ (NSString*) valueWithError:(NSError **)error {

    if (bmobkOpenUDIDSessionCache!=nil) {
        if (error!=nil)
            *error = [NSError errorWithDomain:bmobkOpenUDIDDomain
        ...
        return bmobkOpenUDIDSessionCache;
    }
    ...
    //
    NSString * appUID = [defaults objectForKey:bmobkOpenUDIDAppUIDKey];
    if(appUID == nil)
    {
    ...
    //
    id localDict = [defaults objectForKey:bmobkOpenUDIDKey];
    if ([localDict isKindOfClass:[NSDictionary class]]) {
        ...
        openUDID = [localDict objectForKey:bmobkOpenUDIDKey];
        myRedundancySlotPBid = [localDict objectForKey:bmobkOpenUDIDSlotKey];
        optedOutDate = [localDict objectForKey:bmobkOpenUDIDOOTSKey];
    ...
    for (int n=0; n<kOpenUDIDRedundancySlots; n++) {
        NSString* slotPBid = [NSString stringWithFormat:@"%@%d",bmobkOpenUDIDSlotPBPrefix,n];
#if TARGET_OS_IPHONE || TARGET_IPHONE_SIMULATOR
        ...
        if (slotPB==nil) {
            ...
            NSDictionary* dict = [BmobOpenUDID _getDictFromPasteboard:slotPB];
            NSString* oudid = [dict objectForKey:bmobkOpenUDIDKey];
            ...
            NSString* gid = [dict objectForKey:bmobkOpenUDIDAppUIDKey];
            if (gid!=nil && [gid isEqualToString:appUID]) {
                ...
                if (optedOut) {
                    optedOutDate = [dict objectForKey:bmobkOpenUDIDOOTSKey];
                    optedOut = optedOutDate!=nil;   
                }
            }
        }
    ...
    if (openUDID==nil) {        
        if (mostReliableOpenUDID==nil) {
            ...
            //
            openUDID = [BmobOpenUDID _generateFreshOpenUDID];
        } else {
        ...
        if (localDict==nil) { 
            ...
            [localDict setObject:openUDID forKey:bmobkOpenUDIDKey];
            [localDict setObject:appUID forKey:bmobkOpenUDIDAppUIDKey];
            [localDict setObject:[NSDate date] forKey:bmobkOpenUDIDTSKey];
            if (optedOut) [localDict setObject:optedOutDate forKey:bmobkOpenUDIDTSKey];
    ...
    if (availableSlotPBid!=nil && (myRedundancySlotPBid==nil || [availableSlotPBid isEqualToString:myRedundancySlotPBid])) {
        ...
        //
        if (localDict) {
            [localDict setObject:availableSlotPBid forKey:bmobkOpenUDIDSlotKey];
            saveLocalDictToDefaults = YES;
        }
        ...
        if (openUDID && localDict)
            [BmobOpenUDID _setDict:localDict forPasteboard:slotPB];
    }

    // Save the dictionary locally if applicable
    //
    if (localDict && saveLocalDictToDefaults)
        [defaults setObject:localDict forKey:bmobkOpenUDIDKey];
    ...
    //
    if (optedOut) {
        if (error!=nil) *error = [NSError errorWithDomain:bmobkOpenUDIDDomain
        ...
        bmobkOpenUDIDSessionCache = [[NSString stringWithFormat:@"%040x",0] retain];
        return bmobkOpenUDIDSessionCache;
    }
    ...
    bmobkOpenUDIDSessionCache = [openUDID retain];
    return bmobkOpenUDIDSessionCache;
}
...
+ (void) setOptOut:(BOOL)optOutValue {
    ...
    id dict = [defaults objectForKey:bmobkOpenUDIDKey];
    ...
    if (optOutValue)
        [dict setObject:[NSDate date] forKey:bmobkOpenUDIDOOTSKey];
    else
        [dict removeObjectForKey:bmobkOpenUDIDOOTSKey];

  	// store the dictionary locally
    [defaults setObject:dict forKey:bmobkOpenUDIDKey];
    
    OpenUDIDLog(@"Local dict after opt-out = %@",dict);
    
    // reset memory cache 
   bmobkOpenUDIDSessionCache = nil;
