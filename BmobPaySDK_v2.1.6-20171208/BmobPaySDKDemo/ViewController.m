...
- (IBAction)QQPay:(id)sender {
    [BmobPay payWithPayType:BmobQQpay price:[NSNumber numberWithFloat:[_price.text floatValue]] orderName:_name.text describe:_describe.text result:^(BOOL isSuccessful, NSError *error) {
        if (isSuccessful) {
            _result.text = @"支付成功";
        } else {
             _result.text = error.description;
